// file: src/features/ticket/components/TicketFormPanel.jsx
// Split view form for Create/Edit ticket - fixes form state issues
...
const TicketFormPanel = memo(({
  ...
  const handleChange = useCallback((field, value) => {
    // Auto specific handling for Ticket Number
  ...
  const validate = useCallback(() => {
    ...
    // Ticket Number Validation: 3 Letters + 9 Numbers (Total 12 chars)
    const ticketRegex = /^[A-Z]{3}\d{9}$/;
    if (!formData.ticketNumber?.trim()) {
      newErrors.ticketNumber = 'Ticket Number is required';
    } else if (!ticketRegex.test(formData.ticketNumber)) {
      newErrors.ticketNumber = 'Format must be 3 letters + 9 numbers (e.g. SVR123456789)';
    }
  ...
  return (
    <div className="h-full flex flex-col bg-white dark:bg-zinc-900 overflow-hidden">
      ...
      <form id="ticket-form-panel" onSubmit={handleSubmit} className="flex-1 overflow-y-auto custom-scrollbar p-5 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          <FormSelect
            ...
            options={ticketOptions.responsibilities || []}
          />
          <FormSelect
            label="Assignee"
            icon={User}
            value={formData.assign}
            onChange={v => handleChange('assign', v)}
            options={ticketOptions.assignees || []}
            required={false}
          />
        </div>
...
TicketFormPanel.displayName = 'TicketFormPanel';
