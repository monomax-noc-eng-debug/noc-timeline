...
// ✅ Memoized List: ป้องกันการ Render ซ้ำเมื่อ Parent State เปลี่ยน (เช่น นาฬิกาเดิน)
...
export default function TodayPage() {
  ...
  // ✅ ใช้ useDeferredValue: ให้ UI ตอบสนองทันทีตอนพิมพ์ แต่กรองข้อมูลทีหลัง (แก้กระตุก)
  const deferredSearch = useDeferredValue(searchQuery);

  // ✅ Filter State
  const [hideFinished, setHideFinished] = useState(false);

  // ✅ Filtering Logic (Moved from Hook to Page for performance optimization)
  const filteredMatches = useMemo(() => {
    return matches.filter(match => {
      ...
      // 🚀 Performance: กรองแมตช์ที่จบออกถ้า Hide Finished เปิดอยู่ (ช่วยให้ List สั้นลงและวาดเร็วขึ้น)
    ...
    <div className="flex flex-col h-full w-full bg-white dark:bg-black overflow-hidden">
      ...
      <header className="shrink-0 bg-white dark:bg-[#09090b] border-b border-zinc-200 dark:border-zinc-800 p-3 shadow-sm z-20 transition-all">
        <div className="flex flex-col gap-3">
          ...
          {/* Row 1: Date Nav + Clock + Notification */}
          <div className="flex items-center justify-between gap-2">
            ...
            <div className="flex items-center bg-zinc-100 dark:bg-zinc-900/50 p-0.5 rounded-lg border border-zinc-200 dark:border-zinc-800">
              <button onClick={handlePrevDay} className="w-7 h-7 flex items-center justify-center hover:bg-white dark:hover:bg-zinc-800 rounded-md text-zinc-500 hover:text-black dark:hover:text-white transition-all">
                <ChevronLeft size={16} />
              </button>
              ...
              <button onClick={handleNextDay} className="w-7 h-7 flex items-center justify-center hover:bg-white dark:hover:bg-zinc-800 rounded-md text-zinc-500 hover:text-black dark:hover:text-white transition-all">
              ...
              <button
                ...
                title="Go to Today"
                className={`px-3 h-7 text-[10px] font-black uppercase rounded-md transition-all flex items-center gap-1.5 ${isToday(selectedDate) ? 'text-zinc-400 cursor-default' : 'bg-blue-600 text-white hover:bg-blue-700 shadow-sm'
                  }`}
              >
              ...
              {/* 🛰️ Toggle Hide Finished (Icon Only Next to Today - Always Visible) */}
              <div className="w-px h-4 bg-zinc-200 dark:bg-zinc-800 mx-1" />
              <button
                onClick={() => setHideFinished(!hideFinished)}
                title={hideFinished ? `Show Finished (${stats.finished} matches)` : "Hide Finished"}
                className={`w-7 h-7 flex items-center justify-center rounded-md transition-all ${hideFinished
                  ? 'bg-amber-500 text-white shadow-sm'
                  : 'text-zinc-400 hover:text-blue-600 hover:bg-zinc-50 dark:hover:bg-zinc-800'
                  }`}
              >
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className={`w-8 h-8 flex items-center justify-center rounded-md border transition-all ${notificationEnabled ? 'bg-emerald-50 border-emerald-200 text-emerald-600' : 'bg-white dark:bg-zinc-900 border-zinc-200 dark:border-zinc-800 text-zinc-400'
                  }`}
              >
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-4">
            ...
            <div className="relative group flex-1 sm:flex-none sm:w-64">
              ...
              <input
                ...
                className="w-full h-8 pl-9 pr-3 bg-zinc-100 dark:bg-zinc-900/50 border-none rounded-md text-xs font-medium focus:ring-1 focus:ring-blue-500 transition-all placeholder:text-zinc-500"
              />
            </div>
            ...
            <div className="flex items-center gap-2 border-l pl-2 border-zinc-200 dark:border-zinc-800">
              <button
                ...
                className="w-8 h-8 flex items-center justify-center rounded-md border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 text-violet-500 hover:bg-violet-50 dark:hover:bg-violet-900/10 transition-colors"
                title="Preview Data"
              >
                <FileText size={14} />
              </button>
              ...
              <button
                ...
                className="h-8 px-3 flex items-center gap-1.5 bg-black dark:bg-white text-white dark:text-black rounded-md text-[10px] font-bold hover:opacity-90 transition-opacity shadow-sm whitespace-nowrap"
                title="Add New Match"
              >
                <Plus size={14} /> <span className="hidden lg:inline">Add</span>
              </button>
            </div>
          </div>
        </div>
